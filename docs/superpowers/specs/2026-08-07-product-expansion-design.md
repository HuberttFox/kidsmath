...
现有主题已是马卡龙 base，但存在"白/黑"残留：light 的 `--white: #ffffff`、dark 的灰黑系（`#221f1b`/`#2b2823`/`#2f2b26`/`#38332d`）与纯黑阴影。整体重调：

### 3.1 light 模式
- `--white: #ffffff` → 暖米白 `#fffdf7`（卡面/输入底/次级按钮）
- 按钮文字 `#fff` → `#fffdf7`（暖白，渐变按钮上保持可读）

### 3.2 dark 模式：灰黑系 → 深马卡龙（深奶油棕 + pastel 深调）
| token | 现值（灰黑） | 改为（深马卡龙） |
|-------|------------|----------------|
| `--bg` | `#221f1b` | `#2b211a`（深暖棕） |
| `--card-bg` | `#2f2b26` | `#382a20`（深奶油） |
| `--input-bg` | `#38332d` | `#3f2f23` |
| `--white` | `#2b2823` | `#3a2d22` |
| 阴影 | `rgba(0,0,0,.3/.35)` | `rgba(48,32,20,.35)`（暖褐） |
| 边框 | `rgba(230,221,207,…)` | 深 pastel 调 `rgba(244,210,178,.18)` |

实施时在 `:root` 与 dark 变量块统一调整；对比度 ≥4.5:1 由 impeccable 检测器复核。

### 3.3 全组件核对清单（light+dark 双模式）
body 背景、卡面、输入框/下拉、主/次级按钮、页码 pager、tooltip、hint-box、错误框、legend 图标圆、hero 装饰渐变、阴影、边框——全部走 token，**任何组件不出现纯白/纯黑 hex**。

### 3.4 新页面继承 token
产品页 / 用户 / 会员占位页直接复用 `--bg/--card-bg/--input-bg` 等变量，零新增白/黑。

### 3.5 测试
- CSS 断言：文件中无 `#ffffff`/`#000`/`#fff`（含变量定义）；dark 块无灰黑系 hex 清单
